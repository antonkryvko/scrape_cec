...
def get_party_candidate_info(party_candidate_url):
    ...
    if len(soup.find_all('tr')) <= NUMBER_OF_ROWS_IN_TABLE:
        ...
        party_candidate_info_dict[TITLE_INFO] = soup.select(SELECT_CANDIDATE_INFO_FROM_TABLE)[3].get_text()
    else:
...
def download_party_candidate_photo(photo_url, party_candidate_info_dict):
    ...
    photo_name = party_candidate_info_dict[TITLE_FULLNAME].replace(' ', '_') + '.' + photo_url.split('.')[-1]
    ...
    print('Завантажена фотографія {}'.format(party_candidate_info_dict[TITLE_FULLNAME]))
...
def download_party_programs(program_url, party_candidates_info_dicts):
    ...
    program_name = party_candidates_info_dicts[TITLE_PARTY].replace(' ', '_') + '.' + program_url.split('.')[-1]
    ...
    try:
        ...
        return
    print('Завантажена програма {}'.format(party_candidates_info_dicts[TITLE_PARTY]))
...
def main():
    ...
    party_lists = get_links_to_party_lists()
    for party_counter, party_list in enumerate(party_lists, start=1):
        party_candidates = get_links_to_party_candidates(party_counter, party_list)
        for party_candidate in party_candidates:
            party_candidate_info_dict = get_party_candidate_info(party_candidate)
            download_party_candidate_photo(party_candidate_info_dict[TITLE_PHOTO], party_candidate_info_dict)
            party_candidates_info_dicts.append(pd.DataFrame(party_candidate_info_dict, index=[0]))
